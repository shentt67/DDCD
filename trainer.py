...
from Loss.tsne_tool import do_tsne
...
def load_data(dataset="Cora"):
    ...
    if dataset in ["Cora", "Citeseer", "Pubmed"]:
        ...
        indexes_1 = np.where(data.train_mask == 1)
        indexes_2 = np.where(data.val_mask == 1)
        test_mask[indexes_1] = 0
        test_mask[indexes_2] = 0
...
def evaluate(output, labels, mask):
    ...
    tt = mask.sum().item()
    return float(correct.item()) * 1.000 / float(mask.sum().item())
...
class trainer(object):
    ...
    def train_compute_MI(self):
        ...
        outs = [self.model.inner_sim, self.model.intra_sim, self.model.r_class]
        print("inner_sim, intra_sim, r_class:")
        print(outs)
        ...
        # with torch.no_grad():
        #     logits_tsne = self.model(self.data.x, self.data.edge_index, self.data.y, report_metrics=False)
        #     do_tsne(logits_tsne.detach().cpu(), self.data.y.detach().cpu())

        return acc_test, outs

    def dis_cluster(self):
        self.model.eval()
        with torch.no_grad():
            X = self.model(self.data.x, self.data.edge_index, self.data.y)
        X_labels = []
    ...
    def run_trainSet(self):
        self.model.train()
        logits = self.model(self.data.x, self.data.edge_index, self.data.y, self.data.train_mask)
        logits = F.log_softmax(logits[self.data.train_mask], 1)
        loss = self.loss_fn(logits, self.data.y[self.data.train_mask]) + self.model.loss_DDCD
        self.optimizer.zero_grad()
        loss.backward()
        if self.grad_clip > 0:
            torch.nn.utils.clip_grad_norm(self.model.parameters(), self.grad_clip)
        self.optimizer.step()
        if self.model.loss_DDCD != 0:
            self.model.loss_DDCD = 0
        return loss.item()
    ...
    def run_testSet(self, report_metrics=False):
        ...
        # torch.cuda.empty_cache()
        with torch.no_grad():
            logits = self.model(self.data.x, self.data.edge_index, self.data.y, report_metrics=report_metrics)
